...
def process_time_series_single_ch(raw_data_dir: pathlib.Path, output_dir: pathlib.Path):
    """
    Pre-process CHB-MIT Scalp EEG data and format the output time series as a
    Dataset, with every single channel being a distinct sample.
    ...
    for p in records_with_seizures:
        ...
        for record_path in records_with_seizures[p]:
            ...
            train_input, train_labels = _interleave_channels(train_input,
                                                             train_labels)
            ...
            test_input, test_labels = _get_processed_arrays(edf,
                                                            WINDOW_DURATION,
                                                            test_window_overlap)
            test_input, test_labels = _interleave_channels(test_input,
                                                           test_labels)
...
def _interleave_channels(input: np.ndarray, labels: np.ndarray) -> np.ndarray:
    """
    Split input with S samples containing each C channels into S*C samples
    each containing 1 channel.

    Assuming that the channel dimension is the last of a 3D array:
    (sample, time, channel).
    """
    new_input = np.empty((input.shape[0]*input.shape[2], input.shape[1]),
                         dtype=input.dtype)
    new_input[0::4] = input[:, :, 0]
    new_input[1::4] = input[:, :, 1]
    new_input[2::4] = input[:, :, 2]
